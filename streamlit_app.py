...
if fin.empty:
   ...
   col1.metric("Laba Per Saham", "Rp" + str(int(eps)), deps)
   col2.metric("Harga Buku", "Rp" + str(round(bv)), dbv)
   col3.metric("Nilai Buku", round(pbv,1), dpbv)
   col4.metric("PER", int(per), dper)
   col5.metric("Tunai Per Saham", "Rp" + str(round(tcs)), dtcs)
   col6, col7, col8, col9, col10 = st.columns(5)
   col6.metric("Deviden", str(ceknon(dev)) + "%", ddev)
   col7.metric("Margin Operasi", str(ceknon(om)) + "%", dom)
   col8.metric("Uang Tunai", "Rp" + str(tunai), dnpm)
