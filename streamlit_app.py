...
if fin.empty:
   ...
   vol = vol/1000000
   dfringkas = {'date':date,'kode':kode,'skg':skg,'1ylo':lo,'6m':int(m),'1yhi':hi,'om(%)':int(round(om,2)*100),'dev(%)':int(round(dev,2)*100),'roe(%)':int(round(roe,2)*100),'pos':round(P,0),'cash(M)':int(cash),'opcash(M)':int(opcash),'cash/saham(Rp)':int(tcs),'eps(Rp)':eps,'bv(Rp)':bv,'pbv':pbv,'per':per,'vol(Juta)':vol,'aksiy':aksiy,'aksik':aksik,'user':id}
...
#Pencarian Data
def dataframe():
    caridata = option_menu(None, ['10 Data','Fundamental','Deviden','Index Per Sektor','Finansial'], icons=['arrow-up-square', 'arrow-down-square'], menu_icon="cast", default_index=0, orientation="horizontal")
    if caridata == '10 Data':
