...
if fin.empty:
   ...
   st.subheader(f"EPS : Rp.{int(epsy)} | BV : Rp.{int(bvy)} | PBV : {int(pbvy)} | PER : {int(pery)} | Sektor : {sek}", divider="rainbow")
   ...
   bmper = bm[3]
   bmsek = bm[0]
   st.subheader(f"STANDAR KINERJA EMITEN SEJENIS \n EPS : Rp.{bmeps} | BV : Rp.{bmbv} | PBV : {bmpbv} | PER : {int(bmper)} | SubSektor : {bmsek}", divider="rainbow")
   #deps = eps - bmeps
   #dbv = bv - bmbv
   #dpbv = pbv/bmpbv
