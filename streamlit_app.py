...
if fin.empty:
   ...
   dfringkas = {'date':date,'kode':kode,'skg':skg,'1ylo':lo,'6m':int(m),'1yhi':hi,'om':om,'cash(M)':int(cash),'opcash(M)':int(opcash),'dev':dev,'roe':roe,'pos':P,'eps':eps,'bv':float(bv),'pbv':pbv,'per':per,'vol':vol,'aksiy':aksiy,'aksik':aksik,'user':id}
