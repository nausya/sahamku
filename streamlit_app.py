...
if fin.empty:
   ...
   dfringkas = {'date':date,'kode':kode,'skg':skg,'1ylo':lo,'6m':int(m),'1yhi':hi,'om':om,'dev':dev,'roe':roe,'pos':P,'cash(M)':int(cash),'opcash(M)':int(opcash),'cash/saham':int(tcs),'eps':float(eps),'bv':bv,'pbv':pbv,'per':per,'vol':vol,'aksiy':aksiy,'aksik':aksik,'user':id}
